...
import static org.maayan.maayanproject.model.contstants.TanakhSection.*;
import static org.maayan.maayanproject.utils.StaticUtils.*;
...
public class DBInitializer {
  ...
  private void initTanakh() {
    ...
    Set<String> allBooks = ImmutableSet
      .of(booksInTorah(), booksInProphets(), booksInWritings())
      .stream()
  ...
  private List<Chapter> parseChapters(JSONObject json, String path) {
    ...
    for (int i = 0; i < numChapters; i++) chapters.add(
      parseChapter(
        rawChaptersHebrew.getJSONArray(i),
        rawChaptersEnglish.getJSONArray(i),
        path,
        i + 1
      )
    );
  ...
  private Chapter parseChapter(
    JSONArray hebrewText,
    JSONArray englishText,
    String parentPath,
    int chapterNumber
  ) {
  ...
  private List<Verse> parseVerses(
    ...
    for (int i = 0; i < numVerses; i++) verses.add(
      parseVerse(hebrewText.getString(i), englishText.getString(i), path, i + 1)
    );
    return verses;
  }
  ...
  private Verse parseVerse(
    ...
    verse.setPath(path);
    verse.setFullEnglishText(
      englishText.replaceAll("<i>", "").replaceAll("</i>", "")
    );
  ...
  private String parseSearchableHebrewText(String hebrewText) {
    ...
    for (char c : hebrewText.toCharArray()) if (
      isPlainHebrewCharacter(c)
    ) textBuilder.append(c);
    return textBuilder.toString();
  }

  private TanakhSection determineTanakhSection(String englishName) {
    if (booksInTorah().contains(englishName)) return TORAH; else if (
      booksInProphets().contains(englishName)
    ) return PROPHETS; else return WRITINGS;
  }
}
