...
import static org.maayan.maayanproject.model.contstants.MediaType.AUDIO;
...
class MediaTagGraphQLApiServiceTest {
  ...
  void verses() {
    ...
    result.forEach(
      versesList -> versesList.sort(Comparator.comparing(Verse::getId))
    );
  ...
  void linkedContent() {
    ...
    List<List<MediaContent>> expected = tags
      ...
      .collect(Collectors.toList());
    List<List<MediaContent>> actual = testApi.invokeEndpoint(
      "linkedContent",
      tags
    );
    actual.forEach(
      list -> list.sort(Comparator.comparing(MediaContent::getId))
    );
  ...
  void associateLinkedContentWithMediaTag() {
    ...
    List<MediaContent> actual = testApi.invokeEndpoint(
      "associateLinkedContentWithMediaTag",
      tag,
      input
    );
  ...
  void updateLinkedContentOfMediaTag() {
    var tag = getTag();
    var input = getMediaContentObjects(tag);
    List<MediaContent> mediaContentOfTag = testApi.invokeEndpoint(
      "associateLinkedContentWithMediaTag",
      tag,
      input
    );
    List<MediaContent> updated = new ArrayList<>();
    mediaContentOfTag.forEach(
  ...
  void removeLinkedContentFromMediaTag() {
    ...
    List<MediaContent> toDelete = mediaContentOfTag
      ...
      .collect(Collectors.toList());
    List<MediaContent> deleted = testApi.invokeEndpoint(
      "removeLinkedContentFromMediaTag",
      tag,
      toDelete
    );
  ...
  private List<Verse> versesWithMediaTagsSortedById() {
    return versesWithMediaTagsSortedById(
  ...
  private void setMockMediaContent(MediaTag tag) {
    ...
    var mediaContentObjects = IntStream
